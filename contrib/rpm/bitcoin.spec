...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7572
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7275
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17572
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17275
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 7572
	%{_sbindir}/semanage port -d -p tcp 7275
	%{_sbindir}/semanage port -d -p tcp 17572
	%{_sbindir}/semanage port -d -p tcp 17275
	for selinuxvariant in %{selinux_variants}; do
