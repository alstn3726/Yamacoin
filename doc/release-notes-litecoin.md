YAMAcoin Core version *0.15.1* is now available from:

  <https://download.yamacoin.org/yamacoin-0.15.1/>
...
A number of changes to the way YAMAcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 yamas per kB (0.00001 YAMA). This is to prepare for dropping the minimum transaction fee to 0.1 yamas per kB (0.0001 YAMA) in the next release.
